...
interface FinancialRecordsContextType {
    ...
    updateRecord: (_id: string, newRecord: FinancialRecord) => void;
...
export const FinancialRecordsProvider = ({children} : {children: React.ReactNode;}) => {
    ...
    const response = await fetch("http://localhost:3002/financial-records", {method: "POST", body: JSON.stringify(record), headers: {"Content-Type": "application/json" }, });
    ...
    const updateRecord = async (_id: string, newRecord: FinancialRecord) => {
        // if(!user) return;
        const response = await fetch(`http://localhost:3002/financial-records/${_id}`, {method: "PUT", body: JSON.stringify(newRecord), headers: {"Content-Type": "application/json" }, });
